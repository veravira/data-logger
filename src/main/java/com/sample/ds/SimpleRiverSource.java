    ...
    public Connection connectionForReading() throws SQLException {
        ...
        if (cond) {
            ...
            while (retries > 0) {
                retries--;
                try {
                	logger().info("###################while opening read connection: " + url + " " + user);
                    readConnection = DriverManager.getConnection(url, user, password);
                    logger().info("###################while opening read connection: " + url + " ");
